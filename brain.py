...
class Brain:
    def __init__(self, inputs, clone=False):
        ...
        if not clone:
            ...
            # skpar 2 neuroenr i attention blocket
            for i in [6, 7]:
                self.nodes.append(node.Node(i))
                self.nodes[i].layer = 1
            # skapar bias och output neroen (R^,neuron)
    ...
    def attention(self,node_upp,node_down):
        ...
        #print('Before', node_down.output_value,node_upp.output_value)
        for i in [6,7,8,9]:
       ...
       # print("output value är ",self.nodes[6].output_value)
        ...
        # print('After', node_down.output_value, node_upp.output_value)
